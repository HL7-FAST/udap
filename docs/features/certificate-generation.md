...
- Sandbox "Scenario Walkthrough": one scenario, step by step, through issue, discover, register (client_credentials or authorization_code, with editable scopes), authorize at the IdP (authorization_code only), token, access, and (for `valid`) revoke and verify.
